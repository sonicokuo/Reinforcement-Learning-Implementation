...
class Policy(nn.Module):
    ...
    def calculate_loss(self, gamma=0.999):
        ...
        # print(returns)
        for log_prob_i, R_i in zip(saved_actions, returns):
            
            # print(self.saved_actions[i].log_prob.item(), returns[i])
            policy_losses.append(log_prob_i.log_prob * R_i * current_gamma)
            current_gamma *= gamma
        # print(policy_losses)
...
def train(lr=0.01):
    ...
    for i_episode in count(1):
        ...
        writer.add_scalar('train/training loss', loss, i_episode)
